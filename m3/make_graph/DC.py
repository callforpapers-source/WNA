...
def make_graph(obtained_ap):
	digraph_open = "digraph A {\n"
	for bssid in obtained_ap:
		essid = obtained_ap[bssid]['essid']
		digraph_open += f'"{bssid} - {essid}" [width=1, height=1, shape=diamond, color=salmon2]\n'
		for client in obtained_ap[bssid]['clients']:
			digraph_open += f'"{bssid} - {essid}"->"{client} - ' + obtained_ap[bssid]['clients'][client]['device'] + '"\n'
	digraph_open += '}'
	with open('/tmp/dc', 'w') as fp:
		fp.write(digraph_open)
	os.system(f"cat /tmp/dc | dot -T{outype} > dc.dot")
